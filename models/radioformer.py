...
__all__ = ['RadioFormer']  # model_registry will add each entrypoint fn to this

# import ipdb
from calflops import calculate_flops

class RadioFormer(nn.Module):
    def __init__(self, model="vit_tiny_patch16_224", pretrained=True, is_2D =False,img_size = 112):
        super(RadioFormer, self).__init__()
    ...
    def forward_3d(self,x):
        ...
        if self.model_name == 'vit_base_patch16_224':
            ...
            x = F.adaptive_avg_pool1d(x, 1)[:, :, 0]
        fc_out = self.phase_fc(fc_i)
        x = self.phase_norm(x) # 去不去都要测试

        # cls = F.adaptive_avg_pool1d(x.permute(0,2,1),1)[:,:,0]
        ...
        # cls = F.adaptive_avg_pool1d(cls.permute(0,2,1),1)[:,:,0]
        ...
        # q,_,q_loss = self.codebook(cls[:,0,:])
        #
        # cls_out = self.cls_fc(q)
...
@register_model
def radioformer_resnet18(pretrained=True, **kwargs):
    return RadioFormer(model="resnet18", pretrained=pretrained)


@register_model
def radioformer_vit_tiny(pretrained=True, **kwargs):
    return RadioFormer(model="vit_tiny_patch16_224", pretrained=pretrained)


@register_model
def radioformer_vit_small(pretrained=True, **kwargs):
    return RadioFormer(model="vit_small_patch16_224", pretrained=pretrained)


@register_model
def radioformer_vit_base(pretrained=True, **kwargs):
    return RadioFormer(model="vit_base_patch16_224", pretrained=pretrained)

@register_model
def radioformer_swin_small(pretrained=True, **kwargs):
    return RadioFormer(model="swin_small_patch4_window7_224", pretrained=pretrained,is_2D=True)


if __name__ == '__main__':
    model = radioformer_vit_small()
    flops, macs, params = calculate_flops(model=model,
                                          input_shape=(1,96,3,112,112),
