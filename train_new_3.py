...
from metrics import *
from datasets import MultiPhaseLiverDataset, create_loader
import models
...
# from datasets import MultiPhaseLiverDataset3d
...
parser.add_argument('--val2_anno_file',
                    default='/media/xiaoyubai/raw_data/lld_mmri/data/classification_dataset/labels/labels-validation.txt',
                    type=str)
...
# Misc
parser.add_argument('--seed', type=int, default=44, metavar='S',
...
def main():
    ...
    # dataset_train = MultiPhaseLiverDataset3d(args, is_training=True)
    #
    # dataset_eval = MultiPhaseLiverDataset3d(args, is_training=False)
    #
    # dataset_eval2 = MultiPhaseLiverDataset3d(args, is_training=False, second_val=True)
    ...
    dataset_eval2 = MultiPhaseLiverDatasetUpdate(args, is_training=False, second_val=True)
    ...
    loader_eval2 = create_loader(dataset_eval2,
                                 batch_size=args.validation_batch_size,
                                 is_training=False,
                                 num_workers=args.workers,
                                 distributed=args.distributed,
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            eval_metrics2 = validate(model, loader_eval2, validate_loss_fn, args, amp_autocast=amp_autocast)

            all_metirc = OrderedDict()
            for key in eval_metrics.keys():
                all_metirc[key] = eval_metrics[key]
            for key in eval_metrics2.keys():
                all_metirc['test_' + key] = eval_metrics2[key]
            ...
            if output_dir is not None:
                update_summary(
                    epoch, train_metrics, all_metirc, os.path.join(output_dir, 'summary.csv'),
                    write_header=best_metric is None, log_wandb=args.log_wandb and has_wandb)

            if metric_savers is not None and epoch>=200:
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            output = model(input)
            # loss_phase = loss_fn(output[0], target.repeat(12).view(12,-1).transpose(1,0).reshape(-1))

            # loss_sequence = loss_fn(output[1], target)
            loss_sequence = loss_fn(output.logits, target)
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            # output_fc, output= model(input)

            output = model(input)
            output = output.logits
            output = torch.sum(output, dim=0) / input.shape[0]
            output = output.reshape(1, -1)
            # output_fc = torch.sum(output_fc, dim=0) / input.shape[0]
            # output_fc = output_fc.reshape(1, -1)
        if isinstance(output, (tuple, list)):
            output = output[0]
        # if isinstance(output_fc, (tuple, list)):
        #     output_fc = output_fc[0]

        predictions.append(output)
        labels.append(target)
        # predictions_fc.append(output_fc)

    evaluation_metrics = compute_metrics(predictions, labels, loss_fn, args)
    # evaluation_metrics_fc = compute_metrics(predictions_fc, labels, loss_fn, args)
    if args.local_rank == 0:
        output_str = 'Test:\n'
...
def compute_metrics(outputs, targets, loss_fn, args):
    ...
    targets = torch.cat(targets, dim=0).detach().view(-1)
