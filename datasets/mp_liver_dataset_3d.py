...
class MultiPhaseLiverDataset3d(MultiPhaseLiverDataset):
    def __init__(self, args, is_training=True, num_of_copies=1, **kwargs):
        super().__init__(args, is_training, **kwargs)
    ...
    def __getitem__(self, index):
        ...
        image_slice1 = image[:, :12, :, :].copy()
        image_slice2 = image[:, 1:13, :, :].copy()
        image_slice3 = image[:, 2:14, :, :].copy()
        image_slice1 = image_slice1[:,:,np.newaxis,:,:]
        image_slice2 = image_slice2[:,:,np.newaxis,:,:]
        image_slice3 = image_slice3[:,:,np.newaxis,:,:]
        image_concat = np.concatenate([image_slice1,image_slice2,image_slice3],axis=2)
        image_concat = image_concat.transpose(1,0,2,3,4)
    ...
    def load_mp_images(self, mp_img_list):
        ...
        for img in mp_img_list:
            ...
            # image = resize3D(image[None, ...], self.size)
            mp_image.append(image[None, ...])
            # mp_image.append(image)
