...
class HliverFormer(nn.Module):
    def __init__(self, model="vit_tiny_patch16_224", pretrained=True, is_2D =False):
        ...
        if self.model_name == 'vit_small_patch16_224':
            ...
            self.phase_sequence_cls_token = nn.Parameter(torch.zeros(1, 1, self.vf.embed_dim))
            ...
            self.cls_sequence_norm = norm_layer(self.vf.embed_dim)
            self.cls_fc = nn.Linear(self.vf.embed_dim, 7)
