...
class HliverFormer(nn.Module):
    ...
    def forward_3d(self,x):
        ...
        x = x.view(-1,12,x.shape[1],x.shape[2])
        x = x.permute(0,2,1,3)
        ...
        x = self.phase_sequence_norm(x)
        cls = x[:, 0, :]
        cls = cls.reshape(-1,49,cls.shape[-1])
        ...
        # x = x[:,0:1,:]
        # x = x.reshape(-1, x.shape[1] * 8, x.shape[2])
        #
...
def hilverformer_swin_small(pretrained=True, **kwargs):
    return HliverFormer(model="swin_small_patch4_window7_224", pretrained=pretrained,is_2D=True)


if __name__ == '__main__':
    model = hilverformer_vit_small()
