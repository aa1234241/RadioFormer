...
class HliverFormer(nn.Module):
    def __init__(self, model="vit_tiny_patch16_224", pretrained=True, is_2D =False,img_size = 112):
        ...
        self.phase_merge_block_depth = 1
        ...
        if self.model_name == 'vit_small_patch16_224':
            ...
            # self.phase_fc = nn.Linear(self.vf.num_features*8, 7)
            self.phase_norm = norm_layer(self.vf.num_features)
            self.phase_merge_token = nn.Parameter(torch.zeros(1, 1, self.vf.num_features))
        ...
        self.phase_merge_block = nn.Sequential(*[
            Block(
                dim=self.vf.num_features,
                num_heads=6,
            )
            for _ in range(self.phase_merge_block_depth)])
    ...
    def forward_3d(self,x):
        n_slices = x.shape[1]
        x = x.reshape(-1, 3, x.shape[-2], x.shape[-1]) #batch,slice(12),phase(8),3,112,112->batchx12x8,3,112,112

        # with torch.no_grad():
        x = self.vf.forward_features(x) # batchx12x8,tokens,vf_feature
        n_tokens = x.shape[1]
        # x = x[:,1:,:]
        if self.model_name == 'vit_tiny_patch16_224':
            x = x.permute(0, 2, 1)  # for vit
            ...
            x = F.adaptive_avg_pool1d(x, 1)[:, :, 0]  # for vit
        if self.model_name == 'vit_small_patch16_224':
            x = x.reshape(-1,8,x.shape[1],x.shape[2])
            x = x.permute(0,2,1,3)
            x = x.reshape(-1,x.shape[-2],x.shape[-1])
            x = torch.cat((self.phase_merge_token.expand(x.shape[0], -1, -1), x), dim=1)
            x = self.phase_merge_block(x)
            x = x[:,0,:]
            x = x.reshape(-1,n_tokens,x.shape[-1])
            x = x[:,1:,:]
