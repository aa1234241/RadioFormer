...
all_cases_path = '/media/xiaoyubai/raw_data/lld_mmri/lldmmri_test_set/classification_dataset/images/'
all_cases = os.listdir(all_cases_path)
save_path = '/media/xiaoyubai/raw_data/lld_mmri/data/visual_noregis_full_test/'
if not os.path.exists(save_path):
    os.mkdir(save_path)

label_path = '/media/xiaoyubai/raw_data/lld_mmri/lldmmri_test_set/classification_dataset/labels/'
all_train_cases = np.loadtxt(os.path.join(label_path,'labels_test.txt'), dtype=np.str_)
...
for caseinfo,label in tqdm.tqdm(zip(img_list,lab_list)):
    ...
    for image_name in phase_list:
        ...
        image = resize3D(image[None, ...],(16,128,128))
